...
<form method="post">
	<input type="submit" style="background-color:#cc0000; height:50px; float:right; margin-right:0px; margin-bottom:20px" name ="submit5" value="Logout"/>
</form>
<br>
<?php
session_start();
$usr = $_SESSION['user'];
 $sql = "SELECT labour.name,labour.city,labour.type,labour.regno FROM customer,labour WHERE customer.regno=\"$usr\" AND customer.city = labour.city AND req=1 AND status=1";
 $res = mysql_query($sql);
	 ...
	 ?>
		<tr>
			...
			<td><?php 
				...
				$avg = mysql_query($sql1);
				echo mysql_fetch_array($avg)['ag'];
				?>
			</td>
