...
<?php
	...
	if($row['req']==0 && $row['status'] == 1){
		...
		{
		$creg=$row['cregno'];
		$res=mysql_query("SELECT name FROM customer WHERE regno=$creg");
		}
		else if($row['cregno']==NULL)
		{
		$creg=$row['coregno'];
		$res=mysql_query("SELECT name FROM company WHERE regno=$creg");
		}
		?>
		<center>
		<h1>You have a request from <?php echo mysql_fetch_array($res)['name'];?></h1>
