...
if(isset($_POST['signup']))
{
	...
	$type = mysql_real_escape_string($_POST['specialskill']);
	$regno = mysql_real_escape_string($_POST['lrn']);
	$pwd = md5(mysql_real_escape_string($_POST['password']));

	//mysql_query("INSERT INTO labour(name,email, city, type, regno, pwd) VALUES('$name','$email', '$city','$type','$regno', '$pwd')");
    if(mysql_query("INSERT INTO labour(name,email,city,type,regno,pwd) VALUES('$name','$email','$city','$type','$regno','$pwd')"))
    {
		session_start();
		$_SESSION['user'] = $regno;
...
if (isset($_POST['submit2']))
	{
    $regno = mysql_real_escape_string($_POST['username']);
    $pwd = mysql_real_escape_string($_POST['password']);
    $res = mysql_query("SELECT * FROM labour WHERE regno='$regno'");
