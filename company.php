...
{
    $name = mysql_real_escape_string($_POST['companyname']);
    $city = mysql_real_escape_string($_POST['city']);
    $regno = mysql_real_escape_string($_POST['companyregistrationnumber']);
    $pwd = md5(mysql_real_escape_string($_POST['password']));


    if(mysql_query("INSERT INTO company(name,city, pwd, regno) VALUES('$name', '$city','$pwd', '$regno')"))
    {
		session_start();
	$_SESSION['user'] = $regno;
...
if (isset($_POST['submit3']))
    ...
    $res = mysql_query("SELECT * FROM company WHERE regno='$regno'");
